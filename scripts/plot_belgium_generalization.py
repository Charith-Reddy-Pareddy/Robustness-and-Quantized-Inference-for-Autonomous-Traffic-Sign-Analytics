"""Plots for the BelgiumTSC generalization check: overall accuracy comparison (GTSRB
test set vs. BelgiumTSC, FP32 vs INT8) and per-class F1 on BelgiumTSC."""
...
def main():
    results = json.loads((REPORTS_DIR / "belgium_generalization_results.json").read_text())
    ...
    ax.bar(list(x), fp32_acc, width, label="BelgiumTSC (FP32)", color="#4C72B0")
    ax.bar([i + width for i in x], int8_acc, width, label="BelgiumTSC (INT8)", color="#DD8452")
    ax.set_xticks(list(x))
    ax.set_xticklabels([ARCH_LABELS[a] for a in archs])
    ax.set_ylabel("Accuracy")
    ax.set_ylim(0, 1.0)
    ax.set_title("Accuracy: GTSRB test set vs. BelgiumTSC generalization set")
    ...
    per_class_n = results[arch]["per_class_n"]
    items = sorted(per_class.items(), key=lambda kv: kv[1])
    labels = [f"{GTSRB_CLASS_NAMES[int(c)]} (n={per_class_n[c]})" for c, _ in items]
    values = [v for _, v in items]
    ax.barh(labels, values, color=ARCH_COLORS[arch])
    ax.set_xlabel("F1 score")
    ax.set_title(f"{ARCH_LABELS[arch]}: per-class F1 on BelgiumTSC (FP32)")
    ax.set_xlim(0, 1.0)

    fig.tight_layout()
    out_path = REPORTS_DIR / "belgium_generalization.png"
